...
const testPlugin = function (filePath, expectedFilePath, options) {
  options = options || {};
  return Promise.props({
    actualBuffer: fs.readFileAsync(filePath),
    expectedBuffer: fs.readFileAsync(expectedFilePath)
  })
    .then(({ actualBuffer, expectedBuffer }) => {
      const actualResult = postcss([
...
describe('postcss-var-replace', () => {
  ...
  describe('with at-rules', () => {
    ...
    test('should work with @media, preserving rule order', 'media-query-preserve-rule-order', {
  ...
  describe('with `options.variables`', () => {
    test('should work with JS defined variables', 'js-defined', {
      variables: MOCK_JS_VARIABLES
    });
    test('should work with JS defined important variables', 'js-defined-important', {
      variables: MOCK_JS_VARIABLES
    });
    test(
  ...
  describe('with `options.preserve`', () => {
    test('preserves variables when `preserve` is `true`', 'preserve-variables', { preserve: true });

    test('preserves variables in @media when `preserve` is `true`', 'preserve-variables-in-media', {
      preserve: true
    });

    test("preserves computed value when `preserve` is `'computed'`", 'preserve-computed', {
      preserve: 'computed'
    });
  ...
  describe('missing variable declarations', () => {
    ...
    it('should use string values for `undefined` values, see #22', () =>
      fs.readFileAsync('./test/fixtures/missing-variable-usage.css', 'utf8').then((buffer) => {
        const contents = String(buffer);
        return postcss([cssvariables()])
          .process(contents)
          .then((result) => {
            const { root } = result;
            const fooRule = root.nodes[0];
